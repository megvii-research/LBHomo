...
def weights_self_supervised_and_unsupervised(loss_su, loss_un, stats, loss_weight, apply_constant_weights=False):
    if not apply_constant_weights:
        ...
        ratio = loss_weight['supervision'] / loss_weight['unsupervision']
        ...
        stats['Loss_unsupervision/total'] = (loss_un * u_l_w).item()
        stats['Loss_supervision/total'] = (loss_su * s_l_w).item()
        stats['Loss/total'] = loss.item()
    else:
        loss = loss_weight['unsupervision_constant'] * loss_un + \
               loss_weight['supervision_constant'] * loss_su
        stats['Loss_unsupervision/total'] = (loss_weight['unsupervision_constant'] * loss_un).item()
        stats['Loss_supervision/total'] = (loss_weight['supervision_constant'] * loss_su).item()
        stats['Loss/total'] = loss.item()
    return loss, stats
...
class IdentityLoss:
    def __init__(self, objective, loss_weight, detach_flow_for_warping=True,
                 compute_cyclic_consistency=False,
    ...
    def __call__(self, flow_map, mask_used, estimated_flow_target_prime_to_source,
        ...
        loss_un = self.objective(estimated_flow_target_prime_to_target_through_composition,
                                           flow_map, mask=masks)
        return loss_un
